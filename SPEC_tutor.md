...
### 3.5 小テスト構成（確定仕様・2026-04-21）

本節は §3.2 ステップB および §5.2 の出題構成を**上書き**する確定仕様である (出題数・配点・問題種別はここを正とする)。

#### 3.5.1 問題数と構成

1 回の小テストは **5 問固定**。内訳は以下の通り：

| # | 区分 | 難易度 | 参照範囲 | 配点 |
|---|---|---|---|---|
| Q1 | 基本問題 | 標準 | アップロードした教科書 (ノート含む) の内容**のみ** | 10 |
| Q2 | 基本問題 | 標準 | 同上 | 10 |
| Q3 | 基本問題 | 標準 | 同上 | 10 |
| Q4 | 応用問題 | 中 | 教科書の内容 **＋ 過去の関連学習内容**。応用力を発揮して解く | 15 |
| Q5 | 応用問題 | 高 | 同上。Q4 より一段高度な統合・推論を要する | 5 |

1 回あたりの満点は **50 点** (10×3 + 15 + 5)。

#### 3.5.2 各区分の定義

- **基本問題 (Q1-Q3)**: 当日アップロードされた教科書ページの内容から直接答えられること。過去の単元知識や未習範囲を要求しない。`type = "knowledge"` または `"application"` (教科書内限定の応用) を想定。
- **応用問題・中 (Q4)**: 当日教科書の内容を理解した上で、過去に学習済みの関連単元 (例：既習の前提概念・関連単元・`review_queue` 内の項目など) と組み合わせて解く問題。**解けることを期待する**問題。
- **応用問題・高 (Q5)**: Q4 より一段深い応用・統合・推論を要する問題。**解けなくてよい。より高い応用力の獲得への「気付き」を促すことが目的**。解けなかった場合も否定的に扱わず、「こういう見方もあるんだね」という問いかけ型コメントに留める。

#### 3.5.3 配点設計の意図

- **基本 30 点 / 応用 (中) 15 点 / 応用 (高) 5 点** という配分は、「基本をきちんと押さえることを最優先に評価しつつ、応用への挑戦にも報酬を与える」「しかし応用 (高) が解けないことで達成感を大きく損なわないよう、全体に占める比重を抑える」というバランス設計。
- 「基本 3 問を全問正解すれば 30 点」が当面の安定到達ラインになる。応用 (中) を取れれば 45 点、応用 (高) まで取れれば満点 50 点、という段階感を生徒に意識させる。

#### 3.5.4 ステップB プロンプトへの反映 (§3.2 差分)

§3.2 ステップB の「本日の学習内容から 3 問 / 復習候補から最大 2 問」は本仕様で以下に置き換える：

```
【出題方針】
1. Q1-Q3 は基本問題。本日アップロードされた教科書・ノート画像の内容のみから作成する。
   過去の学習履歴や未習範囲は参照しない。stumble_points のうち「当日範囲で問えるもの」を
   優先して題材にする。各 10 点。
2. Q4 は応用問題 (難易度: 中)。当日の学習内容と、過去の関連学習内容 (links_to_past や
   due_reviews) を組み合わせて解く問題にする。15 点。解けることを想定する。
3. Q5 は応用問題 (難易度: 高)。Q4 より一段高度な統合・推論を要する問題にする。5 点。
   「解けなくてよい。より高い応用力への気付きを促す」問題として設計し、コメント欄には
   到達できなくても落胆させない問いかけ型の補足を用意すること。
4. 全問、小学{{grade}}年生が読める言葉で記述する (応用高も語彙は学年相当)。
```

出力 JSON 形式 (§3.2 ステップB) は以下を追加する：

```json
{
  "tier": "basic | applied_mid | applied_high",
  "points": 10 | 15 | 5,
  "encouragement": "applied_high の場合のみ：解けなくても前向きに受け止められる一言"
}
```

#### 3.5.5 採点・集計への反映

- 採点プロンプト (§3.3) は各問の正誤判定に加え、`tier` と `points` を引き継ぎ、獲得点数 `earned_points` を算出する (基本：正解なら 10、応用中：正解なら 15、応用高：正解なら 5、いずれも誤答は 0)。`partial` は現行通り別途扱う (ただし応用高については可能なら加点寄りに解釈しても良い)。
- `learning_records.total` は 50 固定、`learning_records.score` は獲得点数合計 (0-50)。
- クレジット付与は従来通り「満点 → 既定ボーナス、部分正解 → 比例」などの方針を踏襲するが、**満点条件は 50 点**になる。クレジット計算ロジックは実装時に確認のうえ、応用高を取り切らなくても生徒がモチベーションを失わないバランスに調整する。

#### 3.5.6 応用高問題の表示方針

子への LINE 送信時、Q5 は「★チャレンジ問題 (解けなくてOK)」のようなラベルを付けて表示し、解けなかった場合の採点コメントは励まし寄りに固定する (例: "この考え方は 5〜6 年で詳しくやるよ。ここまで来れたのすごいね")。
...
記法: `[ ]` 未完了 / `[-]` 完了済み (`[x]` ではなく `[-]` を使う)。本節のすべての項目 (サブ項目含む) が `[-]` になった時点で本改修フェーズ完了。

### 8.1 基盤機能

- [-] 同じ単元を繰り返し誤答した場合、自動的に復習問題が生成される (→ `update_review_queue_from_result` + `get_due_reviews` + step B への `due_reviews_detail` 注入)
- [-] 学習履歴が増えると、今日のテストに過去の関連単元が絡んで出題される (→ step A が `recent_topics_summary` を受けて `links_to_past` を生成、step B が復習候補と統合)
- [-] 週次レポートで「今週の成長」「要補強の単元」「来週の方針」が具体的に述べられる (→ `generate_weekly_report` の 4 節構成プロンプト)
- [-] 出題はすべて学年の既習範囲に収まり、未習概念が問われない (→ `STUDYBOT_SYSTEM_PROMPT` + step A プロンプトに「学年の既習範囲のみから出題」指示)
- [-] 誤答の分類（計算ミス / 概念誤解 / 読み違い）が記録され、保護者が傾向を把握できる (→ `question_attempts.mistake_category` + 採点プロンプトの `calc_error/concept_error/read_error/unknown/partial` 分類)

### 8.2 小テスト構成改訂 (§3.5・2026-04-21)

- [-] 景品カタログに `1000 Robux` (1500pt) が追加されている (→ `prizes.json` id=6)
- [-] 小テストは常に 5 問構成 (基本 3 + 応用中 1 + 応用高 1) で出題される
  - [-] migration 0003 で `learning_records.points_total` / `points_earned` および `question_attempts.tier` / `points` / `earned_points` が追加されている (→ `migrations/0003_points_system.sql`, T1)
  - [-] Step B プロンプトが `[basic×3, applied_mid×1, applied_high×1]` 構成を必須で要求する (→ `_STEP_B_PROMPT_TEMPLATE`)
  - [-] Claude 応答が 3 問未満の場合のみ 1 回 retry し、3〜5 問は許容、6 問以上は先頭 5 問に切り詰める (→ `generate_questions_step_b`, T5/T6)
  - [-] 正規化で `tier`/`points`/`encouragement` を whitelist・enum validate・int clamp・length truncate を通す (プロンプトインジェクション防御の継続、A-7/P2-2 方針) (→ `_normalize_questions`, T2/T3/T4/T13/T15)
  - [-] 子への出題 UI に「基本問題」「応用問題 (中)」「★チャレンジ (応用・高)」のセクションと各問の配点が表示される (→ `format_child_quiz_sections`, T11)
- [-] 配点に基づくポイント制度 (10×3 + 15 + 5 = 満点 50) でクレジットが付与される
  - [-] `finalize_grading` が tier × 正誤で `earned_points` を all-or-nothing で集計する (→ `finalize_grading`, T7/T8/T9)
  - [-] `credits` 加算が `earned_points` ベースで、`learning_records.points_earned` 更新と同一トランザクションで atomic に行われる (B-4/P1-2 方針) (→ `finalize_grading` 内 `sqlite3.connect` 1 トランザクション, T16)
  - [-] 満点時クレジット加算が 50cr を維持する (旧 `CREDIT_PER_CORRECT × 5 問` と等価で経済不変) (→ T7 で earned=50/balance=50 確認)
  - [-] 日次・週次レポートに「獲得点 / 50」が併記される (旧レコード `points_total=0` は従来表示を維持) (→ `_format_record_score_line` + `_build_daily_report_prompt`, T14)
- [-] 応用問題 (高) は「解けなくてよい・気付きを促す」問題として児童に提示される
  - [-] 出題 UI で Q5 に「★チャレンジ (解けなくて OK)」ラベルが付く (→ `_TIER_SECTION_TITLE[TIER_APPLIED_HIGH]`, T11)
  - [-] Q5 誤答時の採点コメントが Claude 生成の `encouragement` (問いかけ型・励まし) に置換される (→ `format_child_grading_result`, T12)

### 8.3 品質担保 (本改修範囲)

- [-] `test_bot.py` に §3.5 対応の新規テスト (T1〜T16) が追加され、全テスト (既存 + 新規、計 75 件) が通過する
- [-] LINE 実機 smoke テスト (`SMOKE_quiz_v2.md` ローカル手順) が成功する
- [-] 新規コードが IMPROVEMENT_PROPOSAL の既確立方針 (A-4 ログ衛生 / A-7 プロンプトインジェクション / B-4 トランザクション / B-7 migration / C-2 SQL パラメータ化) に準拠している (→ PII を含まない件数・tier 分布のみログ / `_normalize_questions` whitelist / `finalize_grading` 単一トランザクション / `0003_points_system.sql` 冪等適用 / すべて placeholder SQL)
