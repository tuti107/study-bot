...
def run_migrations() -> list[int]:
    """migrations/NNNN_*.sql をバージョン順に適用する。既に適用済みのものは skip。
    戻り値: このセッションで新規適用した version のリスト。"""
    applied: list[int] = []
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if os.path.isdir(MIGRATIONS_DIR):
            for fname in sorted(os.listdir(MIGRATIONS_DIR)):
                m = _MIGRATION_NAME_RE.match(fname)
                if not m:
                    continue
                version = int(m.group(1))
                entries.append((version, fname, os.path.join(MIGRATIONS_DIR, fname)))
        for version, fname, path in entries:
            if version in done:
                continue
            with open(path, encoding="utf-8") as f:
                sql = f.read()
            # executescript は実行前に暗黙 COMMIT を発行するため、
            # マイグレーション内の各文はそれぞれ独立して適用される想定。
            # データ変換を伴う将来のマイグレーションでは、文単位で BEGIN/COMMIT を書く。
            ...
            logger.info("migration_applied version=%d name=%s", version, fname)
    return applied


def init_db() -> None:
    """後方互換のエイリアス。内部でマイグレーションランナーを呼ぶ。"""
    run_migrations()
